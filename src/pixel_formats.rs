...
pub fn transform(pixels: &[u8], input: &PixelFormat, output: &PixelFormat) -> Vec<u8> {
    ...
    while i < pixels.len() {
        ...
        match &output.color_spec {
            ...
            ColorSpecification::ColorMap(cmap) => {
                let ir = convert(ir_raw, in_cf.red_max, 255) as i16;
                let ig = convert(ig_raw, in_cf.green_max, 255) as i16;
                let ib = convert(ib_raw, in_cf.blue_max, 255) as i16;

                let color_index = VESA_VGA_256_COLOR_PALETTE
                    .iter()
                    .enumerate()
                    .min_by_key(|(_, (pr, pg, pb))| {
                        // min by RGB distance formula. not the best by today's
                        // understanding of color perception, but neither is
                        // the VGA palette -- for now we're just supporting
                        // the protocol without crashing or misbehaving here.
                        (*pr as i16 - ir).pow(2)
                            + (*pg as i16 - ig).pow(2)
                            + (*pb as i16 - ib).pow(2)
                    })
                    .unwrap() // constant array of nonzero size always has a min
                    .0;
...
// for compatibility with VNC clients configured to requesting ColorMap modes,
// we'll punt for now (rather than doing anything adaptive) and use this look-up table
// https://upload.wikimedia.org/wikipedia/commons/6/66/VGA_palette_with_black_borders.svg
#[rustfmt::skip]
const VESA_VGA_256_COLOR_PALETTE: [(u8, u8, u8); 256] = [
